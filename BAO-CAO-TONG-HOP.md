# BetterDay-Tool — Báo cáo phân tích & Đề xuất nâng cấp cho AI Challenge HCMC
...
9. [3 giải pháp sáng tạo nâng cấp BetterDay](#9-3-giải-pháp-sáng-tạo-nâng-cấp-betterday)
...
**BetterDay** là hệ thống tìm kiếm & truy xuất video mã nguồn mở, dựa trên nền tảng SOMHunter, từng trưng bày tại **AI Challenge 2023 (TP.HCM)**. Mục đích: cho phép tìm video bằng **truy vấn ngôn ngữ tự nhiên** (semantic search).
...
│   ├── pom.xml
│   └── src/main/java/com/challenge/betterday/
│       ├── Application.java
...
```
mvn package → betterdaytool.jar
        ↓
java -jar betterdaytool.jar
        ↓
...
**Đây gần như chính xác là kiến trúc BetterDay đang nhắm tới**, và là **baseline phổ biến nhất trong cuộc thi**. Tức là làm theo cách này → chỉ ngang đám đông, không thắng được.
...
- **Translation bridging** là pattern *thực dụng* mà BetterDay (và đa số đội) đang bỏ lỡ.
...
**Mạnh:** tận dụng được code BetterDay; OCR Qwen-VL xử lý chữ tiếng Việt tốt; interpretable cho operator; search siêu nhanh.
...
## 9. 3 giải pháp sáng tạo nâng cấp BetterDay
...
| Điểm yếu                                     | BetterDay  | Cả ngành VBS                                          |
| ----------------------------------------------- | ---------- | ------------------------------------------------------- |
...
**Diff vs BetterDay**: thay class `Video` đơn lẻ → 6 class song song link nhau qua `frame_id`; thêm Python AI service cho ingest; React thêm slider.
...
**Diff vs BetterDay**: thêm `SessionAdapterService` (in-memory `Map<sessionId, δ>`); React thêm nút ✓/✗ → POST `/api/feedback` → re-rank tự động.
...
**Diff vs BetterDay**: thêm Neo4j (hoặc PostgreSQL recursive CTE) song song Weaviate; thêm step `generate_15_queries` trong ingest; backend thêm `GraphQueryService` translate LLM-output → Cypher.
...
**Áp dụng cho BetterDay**: phải thay vectorizer mặc định của Weaviate bằng **CLIP-Vi multilingual** hoặc **BLIP-2** + bridging translation.
...
**Áp dụng cho BetterDay**: hiện tại chỉ có 1 tầng (Weaviate semantic search). Phải thêm Elasticsearch index tiền OCR/ASR + 1 tầng rerank cuối.
...
**Áp dụng cho BetterDay**: trước khi index → preprocess data + translation bridging tiếng Việt → tiếng Anh.
...
Weaviate của BetterDay đã có HNSW nhưng đang dùng default config → có thể tăng 5-10× tốc độ chỉ bằng tuning.

---

## 11. Kiến trúc combo & lộ trình 8 tuần

### Kiến trúc kết hợp cả 3 sáng tạo trên xương BetterDay

```
              ┌─────────────────────────────────┐
              │   React UI (BetterDay v2)       │
...
| Tính năng           | NaiveNotNice (2024) | TycheVid (#1 2024)  | RAPID (giải 3, SOICT'24) | **BetterDay v2 (đề xuất)** |
| --------------------- | ------------------- | ------------------- | ------------------------- | ----------------------------------- |
...
| ----- | --------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------- |
| 1-2   | Refactor BetterDay: tách Python AI service, sửa GraphQL bug, dựng ingest pipeline cơ bản.**Thêm envit5-translation bridging** ngay từ đầu. | RAPID                                     |
...
> Đừng chạy đua CLIP+rerank như mọi đội — hãy **đập vỡ embedding monolithic (DMAR)**, **làm hệ thống tự học trong session (Session Adapter)**, và **lên một tầng cao hơn về thời gian (Event Graph)**. Ba thứ này chưa có hệ VBS nào kết hợp đầy đủ, đủ "mới" để ấn tượng, đủ thực tế để chạy được trong 8 tuần dựa trên BetterDay sẵn có.
...
**Models cần tải sẵn** cho BetterDay v2:
